...
> 현재 대화에서 공유된 GEO 분석 결과나 `/geo audit` 실행 결과를 참조한다.
> 분석 결과가 없는 경우 사용자에게 `/geo audit [URL]`을 먼저 실행하도록 안내한다.
...
제안서 작성 전에 아래 명령어로 분석을 먼저 실행하세요:

  /geo audit https://[도메인]         ← 전체 감사
  /geo technical https://[도메인]     ← 기술 SEO (L3)
  /geo schema https://[도메인]        ← 스키마 현황 (L3)
  /geo llmstxt https://[도메인]       ← llms.txt 진단 (L3)

분석 완료 후 다시 /geo proposal 을 실행하세요.
```
...
| 분석 | 명령어 | 권고 시점 |
|---|---|---|
| [미실행 분석명] | `/geo [command] https://[도메인]` | Sprint [N] 완료 후 |
...
| 시점 | 명령어 | 목적 |
|---|---|---|
| Sprint 1 완료 후 | `/geo audit https://[도메인]` | 즉시 개선 항목 효과 확인 |
| Sprint 2 완료 후 | `/geo report https://[도메인]` | 중간 성과 보고서 생성 |
| Sprint 3 완료 후 | `/geo audit https://[도메인]` | 최종 GEO 점수 측정 |
```
