...
- sitemap.xml 또는 robots.txt 내 Sitemap 지시가 확인되지 않음
...
| OG title | 올리브영 온라인몰 |
| OG description | 대한민국 NO.1 헬스&뷰티 스토어 OLIVEYOUNG |
| Canonical | 미감지 |
| hreflang count | 0 |
| JSON-LD count | 0 |
...
1. meta description 부재 또는 초기 HTML에서 감지되지 않음
2. canonical 링크가 초기 HTML에서 감지되지 않음
3. llms.txt가 감지되지 않음
4. JSON-LD 구조화 데이터가 초기 HTML에서 감지되지 않음
5. sitemap.xml 또는 robots.txt 내 Sitemap 지시가 확인되지 않음
