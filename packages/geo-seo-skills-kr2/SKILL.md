...
이 패키지에서 "근거 기반"은 내부에서 만든 표나 해석만을 뜻하지 않는다. 핵심
주장은 외부의 공식 문서, 표준, 학술/실증 연구, 공학적으로 검증 가능한 원리에
연결되어야 한다.
...
| 순위 | 근거 유형 | 사용 방식 |
|---|---|---|
| P0 | 표준·프로토콜 | robots.txt, HTTP, sitemap, hreflang 같은 공학적 기준 |
| P1 | 공식 문서 | OpenAI, Anthropic, Google, Apple, NAVER, Baidu 등 플랫폼 동작 |
| P2 | 학술·실증 연구 | crawler compliance, retrieval, structured data 한계, 측정 설계 |
| P3 | 전문가 기술 보고서 | 보조 해석 |
| L | 한국/지역 자료 | 한국 서비스·시장·기관 맥락. 단, 일반 원리의 1차 근거로 사용하지 않음 |

보고서와 권고는 `references/evidence-boundary.md`의 `Measured`, `Readiness`,
`Heuristic`, `Manual Fallback`을 구분해야 한다. readiness나 heuristic을 실제 AI
인용/노출 측정값으로 표현하지 않는다.

외부 근거 색인은 `references/source-index.md`를 사용한다. 기능 매칭과 실행 계약은
`references/function-matching-matrix.md`를 기준으로 한다.

### 현실 가능성 게이트

서브스킬은 아래 세 종류의 주장만 현실적으로 가능한 주장으로 취급한다.

| 현실 주장 유형 | 필요 근거 | 허용 표현 |
|---|---|---|
| 관측 가능한 사이트 신호 | 실제로 읽은 HTML, robots.txt, sitemap, schema, hreflang, HTTP 응답 | "준비 신호가 있다/없다" |
| 공식 문서 기반 readiness | RFC/W3C/Sitemaps.org, Google/OpenAI/Anthropic/NAVER 등 공식 문서 | "공식 문서 기준으로 설정이 맞다/부족하다" |
| 직접 실측 | 저장된 플랫폼 출력, citation URL, screenshot, log, referral, conversion | "이 캡처/로그 범위에서 관측됐다" |

직접 실측이 없으면 "AI가 실제로 인용한다", "노출된다", "순위가 오른다",
"플랫폼이 반드시 채택한다"는 표현을 금지한다. 이 경우 `Readiness` 또는
`Heuristic`으로만 말한다.
...
분석할 사이트 주소와 함께 명령을 입력하세요.
  ...
  /geo citability <주소> 내 글의 AI 인용 준비/가능성 휴리스틱 점수
  /geo crawlers <주소>   AI 봇이 사이트를 볼 수 있는지 확인
  /geo brands <주소>     브랜드 언급 현황 (YouTube, Reddit, Wikipedia 등)
  /geo platforms <주소>  ChatGPT·Perplexity·Google AIO별 readiness 분석
...
[ GEO-SEO 분석 도구 — 개발자 모드 ]
  ...
  /geo brands <url>      브랜드 멘션 스캔
  /geo platforms <url>   플랫폼별 readiness/최적화 신호
  ...
  /geo multilang <url>         다국어 사이트 GEO 진단 (hreflang·번역 품질·언어별 readiness)
  /geo lang-platform <url>     언어별 AI 플랫폼 최적화 분석
  /geo proposal <도메인>       GEO 개선 제안서 자동 생성
  /geo prospect <url>...       잠재 고객 GEO 빠른 스캔 (배치 지원)
  /geo realtime <url>          브라우저 캡처 기반 인용 관측 — Code 전용 (Playwright 필요)
  /geo realtime <url> --cp     CP 콘텐츠 기반 질문 자동 생성 후 캡처
  /geo tracker <도메인>         Gap 시계열 추적 — Code 전용 (BASELINE 파일 필요)
  /geo batch <url1> <url2> ...  복수 도메인 배치 스캔 — Code 전용
  /geo-code init               확장 계층 환경 점검 — Code 전용
  /geo-code pipeline <url>     감사·실측·추적 자동 파이프라인 — Code 전용
  /geo-code status [도메인]    로컬 분석 파일·BASELINE·CP 연동 상태 확인
...
| `/geo-code` | geo-code (확장 계층 오케스트레이터 — Code 전용) | L3 |
...
이유:       LLM용 안내 신호를 제공하는 제안 파일입니다.
            없을 경우 `llms.txt` 기반 안내 신호는 제공되지 않습니다.
...
| 명령어 | 필요 도구 | 설명 |
|---|---|---|
| `/geo realtime` | Playwright + Node.js | 브라우저 캡처 기반 인용 관측 |
| `/geo tracker` | 로컬 파일 시스템 | BASELINE 기반 Gap 시계열 추적 |
| `/geo batch` | Bash + WebFetch | 복수 도메인 간이 GEO 스캔 비교 |
| `/geo-code` | Bash + Playwright + 로컬 파일 시스템 | init·pipeline·status 오케스트레이션 |
...
| 순서 | 분석 영역 | 서브스킬 |
|---|---|---|
| 1 | AI readiness 신호 (인용 가능성, 크롤러, llms.txt, 브랜드) | geo-citability + geo-crawlers |
...
- **L1 (마케팅 담당자):**
  ...
  1. AI 봇 차단 (긴급)
     ClaudeBot 접근 readiness가 낮은 상태입니다.
     → 개발팀 요청: "robots.txt에서 ClaudeBot 차단 해제"

  2. 구조화 데이터 없음 (중요)
     검색/AI 시스템이 이해할 구조화 신호가 부족합니다.
...
이 점수는 별도 실측 evidence가 없으면 `heuristic readiness score`이다. 즉, 실제
AI 플랫폼에서 인용되었거나 노출되었다는 `Measured` 값이 아니라, 외부 공식 문서와
공학적 원리에 비추어 현재 사이트가 AI/검색 시스템에 이해·수집·인용될 준비가
얼마나 되어 있는지를 추정하는 의사결정 점수다.
