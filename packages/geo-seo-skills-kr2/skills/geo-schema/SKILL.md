...
description: >
  ...
  다국어 사이트(SITE_LANGS 2개 이상)는 inLanguage, availableLanguage, translationOfWork, workTranslation
  등 다국어 스키마 타입을 추가로 생성한다.
  L3(개발자) 전용 스킬.
  트리거: "스키마", "JSON-LD", "구조화 데이터", "schema", "inLanguage", "다국어 스키마", "/geo schema".
audience: L3
...
| **inLanguage** | **다국어 높음** | **모든 언어 버전 페이지** | **페이지 언어를 AI에 명시 — AI 크롤러 언어 인식 핵심** |
| **availableLanguage** | **다국어 높음** | **Organization / 서비스 페이지** | **사이트 지원 언어 전체 선언** |
| **translationOfWork** | **다국어 보통** | **번역 페이지** | **번역본 → 원본 연결 (AI 중복 방지)** |
| **workTranslation** | **다국어 보통** | **원본 페이지** | **원본 → 번역본 연결** |
| **LocationFeatureSpecification** | **다국어 조건부** | **LocalBusiness 페이지** | **지역별 서비스 가용 언어 표기** |
...
| speakable | 지원 표면에서 요약 후보로 식별할 핵심 구간이 있는 페이지 |
...
| inLanguage | SITE_LANGS 2개 이상 — 모든 언어 버전 페이지 |
| availableLanguage | SITE_LANGS 2개 이상 — Organization·서비스 페이지 |
| translationOfWork | SITE_LANGS 2개 이상 — 번역 페이지 (원본이 존재하는 경우) |
| workTranslation | SITE_LANGS 2개 이상 — 원본 페이지 (번역본이 존재하는 경우) |
| LocationFeatureSpecification | LocalBusiness 포함 + SITE_LANGS 2개 이상 — 다국어 서비스 가용 언어 표기 |
...
FAQ 형식 콘텐츠를 구조화된 개별 답변 후보로 식별할 수 있게 정리한다.
...
## 다국어 스키마 타입 (SITE_LANGS 2개 이상인 경우)

---

### inLanguage — 페이지 언어 명시

각 언어 버전 페이지에 `inLanguage`를 추가하여 AI 크롤러가 언어를 정확히 인식하도록 한다.
Article·WebPage·BlogPosting에 삽입한다.

```html
<!-- 한국어 페이지 (https://example.com/ko/) -->
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "WebPage",
...
> `inLanguage` 값은 hreflang 태그의 언어-지역 코드와 일치시킨다 (예: `ko-KR`, `en-US`, `ja-JP`).

---

### availableLanguage — 사이트 지원 언어 전체 선언

Organization 스키마에 `availableLanguage`를 추가하여 사이트가 지원하는 언어 전체를 AI에게 알린다.
...
> `availableLanguage` 배열에 SITE_LANGS 각 언어를 모두 포함한다.

---

### translationOfWork / workTranslation — 번역 관계 선언

원본 페이지와 번역 페이지를 연결하여 AI 크롤러가 중복 콘텐츠로 판단하지 않도록 한다.

```html
<!-- 원본 페이지 (한국어): 번역본을 workTranslation으로 선언 -->
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Article",
  "url": "https://[도메인]/ko/[글경로]/",
  "inLanguage": "ko-KR",
  "headline": "[한국어 제목]",
  "workTranslation": [
    {
      "@type": "Article",
      "url": "https://[도메인]/en/[글경로]/",
      "inLanguage": "en-US",
      "headline": "[English Title]"
    },
    {
      "@type": "Article",
      "url": "https://[도메인]/ja/[글경로]/",
      "inLanguage": "ja-JP",
      "headline": "[日本語タイトル]"
    }
  ]
...
<!-- 번역 페이지 (영어): 원본을 translationOfWork로 선언 -->
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Article",
...
> hreflang 태그와 병행 사용한다. hreflang은 크롤러 탐색 신호, translationOfWork는 콘텐츠 관계 선언으로 역할이 다르다.

---

### LocationFeatureSpecification — 지역별 서비스 가용 언어

LocalBusiness 스키마에 `amenityFeature`를 추가하여 해당 매장·사무소에서 제공하는 언어를 AI에 명시한다.
다국어 로컬 비즈니스(음식점·병원·매장 등)가 언어별 검색/AI readiness 신호를 제공하도록 돕는다.
...
**사용 기준:**
- `value: true` — 해당 언어로 서비스 제공 가능
- `value: false` — 미제공 (선택적 — 생략해도 무방)
- `name` 값은 영어 언어명 사용 (Korean·English·Japanese·Chinese·Spanish 등)
- SITE_LANGS에 포함된 언어와 일치시킨다

> LocalBusiness 타입이 없는 사이트(SaaS·미디어·에이전시 등)는 이 스키마를 생략한다.
> availableLanguage(Organization 수준 선언)와 역할이 다르다 — availableLanguage는 사이트 전체 언어, LocationFeatureSpecification은 특정 물리적 위치의 서비스 언어를 명시한다.
...
| Person | 해당 없음 / ✅ / ❌ | — | — |
| inLanguage (다국어) | ✅ 있음 / ❌ 없음 / 해당 없음 | — | — |
| availableLanguage (다국어) | ✅ 있음 / ❌ 없음 / 해당 없음 | — | — |
| translationOfWork (다국어) | ✅ 있음 / ❌ 없음 / 해당 없음 | — | — |
| workTranslation (다국어) | ✅ 있음 / ❌ 없음 / 해당 없음 | — | — |
| LocationFeatureSpecification (다국어) | ✅ 있음 / ❌ 없음 / 해당 없음 | — | — |

> 다국어 행은 SITE_LANGS 2개 이상인 경우에만 표시한다. 단일 언어 사이트는 삭제한다.
> LocationFeatureSpecification 행은 LocalBusiness 타입이 있는 경우에만 표시한다.
