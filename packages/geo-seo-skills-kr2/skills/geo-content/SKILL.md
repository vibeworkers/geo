...
description: >
  ...
  다국어 사이트(SITE_LANGS 2개 이상)는 번역 품질 평가 모듈을 추가 실행한다.
  OUTPUT_LANG에 따라 한국어·영어·일본어·중국어·스페인어 보고서를 생성한다.
  모든 레벨에서 동일하게 분석하며 출력 방식만 달라진다.
  트리거: "콘텐츠 품질", "E-E-A-T", "신뢰도", "저자", "번역 품질", "content", "/geo content".
audience: L1, L2, L3
...
> 파일명: OUTPUT_LANG=ko → `GEO-콘텐츠-분석.md` / 그 외 → `GEO-content-[OUTPUT_LANG].md`
...
| 점수 | 등급 | 상태 |
|---|---|---|
| 80–100 | 우수 | E-E-A-T/readiness 신호가 충분함 |
| 60–79 | 양호 | 기본 신뢰도 확보, 일부 보완 필요 |
| 40–59 | 보통 | 전문성·권위성 강화 필요 |
| 20–39 | 미흡 | E-E-A-T 전반 개선 필요 |
| 0–19 | 위험 | 콘텐츠 신뢰/readiness 신호가 매우 부족함 |

---

### 5-1단계: 번역 품질 평가 (SITE_LANGS 2개 이상인 경우에만 실행)

> 단일 언어 사이트는 이 단계를 건너뛴다.

SITE_LANGS의 각 언어 버전 페이지를 기준 언어(SITE_LANGS[0]) 페이지와 비교한다.

#### 번역 완성도

| 항목 | 확인 방법 | 기준 |
|---|---|---|
| 콘텐츠 길이 비율 | 번역 페이지 텍스트 길이 ÷ 기준 언어 텍스트 길이 | 70% 이상 권장 |
| 섹션 수 일치 | H2·H3 제목 수 비교 | 기준 언어 대비 80% 이상 |
| 이미지·표 포함 | 기준 언어와 동일한 미디어 요소 존재 여부 | 동일 권장 |
| hreflang 등록 | 해당 언어 버전 hreflang 태그 존재 | 필수 |

#### 번역 품질 신호

**기계 번역 의심 패턴 (감점 요인):**
- 어색한 문장 구조 — 원본 언어 문법을 그대로 옮긴 직역
- 고유명사 미번역 또는 오번역 (제품명, 지명 등)
- 날짜·수치·통화 형식 미현지화 (예: 한국 원화를 달러 환산 없이 그대로 사용)
- 문화적 맥락 부재 (현지 관용구·사례·플랫폼 미반영)
- HTML 엔티티 문자 깨짐 또는 레이아웃 이상

**현지화 완성도 신호 (가점 요인):**
- 날짜 형식 현지화: en=`May 9, 2026` / ja=`2026年5月9日` / zh=`2026年5月9日` / es=`9 de mayo de 2026`
- 언어별 검색 플랫폼 언급: en=Google·Reddit / ko=네이버·카카오 / ja=Yahoo! Japan / zh=바이두
- 현지 통화·단위 표기: KRW(₩)·USD($)·EUR(€)·JPY(¥)
- 현지 고객 사례·인용 포함

#### 번역 품질 등급

```
EXCELLENT: 콘텐츠 길이 90%+, 현지화 완성, 기계 번역 의심 없음
GOOD     : 콘텐츠 길이 70~89%, 기본 현지화, 경미한 어색함
FAIR     : 콘텐츠 길이 50~69%, 현지화 미흡, 기계 번역 부분 의심
POOR     : 콘텐츠 길이 50% 미만 또는 기계 번역 전체 의심
MISSING  : 번역 페이지 없음 또는 기준 언어 콘텐츠 그대로 노출
```

**번역 품질 소계 (언어별):** [EXCELLENT / GOOD / FAIR / POOR / MISSING]
...
"검색/AI 시스템이 이해할 신뢰 신호가 얼마나 갖춰졌는가"를 중심으로 설명한다.
...
[한 줄 요약]
예) "글쓴이 정보가 없어 검색/AI 시스템이 활용할 신뢰 신호가 부족한 상태입니다."
...
현재: 저자 바이라인 없음
영향: Google 품질 프레임워크 기준 신뢰 신호가 약하고, 출처 식별 신호가 불명확함
해결:
  1. Person 스키마 추가:
  ```json
  {
