...
name: geo-citability
description: >
  AI 인용 준비/가능성 휴리스틱 평가. 페이지가 ChatGPT, Perplexity,
  Google AI Overviews 등 AI 검색 표면에서 인용될 준비 신호를 갖췄는지 진단한다.
  직접 답변 구조, 콘텐츠 권위성, 기술 인용 신호, 브랜드 명확성 4개 차원으로 평가한다.
  직접 플랫폼 캡처가 없으면 실제 인용률이나 노출률을 주장하지 않는다.
...
# geo-citability — AI 인용 준비/가능성 휴리스틱 평가

> 실행 시 USER_LEVEL을 확인한다. 설정되지 않은 경우 레벨 선택을 먼저 요청한다.
> 보고서 언어: OUTPUT_LANG에 따라 작성한다 (ko=한국어·기본, en=영어, ja=일본어, zh=중국어, es=스페인어).
> 파일명: OUTPUT_LANG=ko → `GEO-인용가능성-분석.md` / 그 외 → `GEO-citability-[OUTPUT_LANG].md`

> 현실 가능성 경계: 이 스킬은 기본적으로 `Heuristic`이다. 실제 플랫폼 출력,
> citation URL, screenshot, log가 함께 저장되지 않으면 "실제로 인용된다" 또는
> "노출된다"라고 말하지 않는다.
...
### 2단계: 인용 준비/가능성 4개 차원 평가

각 차원을 0–25점으로 평가한다. 합산 점수(0–100)가 인용 준비 점수다.

#### 직접 답변 구조 (Direct Answer Structure) — 0~25점

LLM/검색 시스템이 페이지에서 답변 후보를 추출하기 쉬운 구조인지 평가한다.
...
공식 품질 프레임워크와 출처성 원리에 비추어 신뢰 신호가 충분한지 평가한다.
...
AI/검색 시스템이 출처 후보를 명확히 식별할 수 있는지 평가한다.
...
```
인용 준비 점수 = (직접답변구조 × 0.35) +
                  (콘텐츠권위성 × 0.30) +
                  (기술인용신호 × 0.20) +
                  (브랜드명확성 × 0.15)
```

가중치는 실측 causal model이 아니라 이 패키지의 휴리스틱 평가 가중치다.

**점수 등급표**

| 점수 | 등급 | 상태 |
|---|---|---|
| 80–100 | 우수 | 인용 준비 신호가 매우 강함 |
| 60–79 | 양호 | 기본 인용 구조를 갖췄으나 개선 여지 있음 |
| 40–59 | 보통 | 주요 인용 준비 신호 일부 미흡 |
| 20–39 | 미흡 | 인용 준비 신호가 낮아 개선 필요 |
| 0–19 | 위험 | 인용 준비 신호가 매우 부족함 |
...
AI 검색에서 인용될 준비 상태를 일상 언어로 전달한다.
"이 글이 ChatGPT나 AI 검색의 답변 후보로 쓰이기 쉬운 구조인가"를 중심으로 설명한다.
기술 용어 없이, 개선 방향은 누가 무엇을 해야 하는지로 안내한다.

```markdown
# [페이지 제목] AI 인용 준비 상태 분석

분석일: [날짜]  |  분석 URL: [URL]

---

## AI 인용 준비 상태: [높음 / 보통 / 낮음 / 매우 낮음]
...
| 확인 항목 | 현황 | 설명 |
|---|---|---|
| AI 답변 후보로 쓰이기 쉬운 구조인가요? | 좋음 / 주의 / 위험 | [한 줄] |
| 신뢰할 수 있는 데이터와 출처가 있나요? | 좋음 / 주의 / 위험 | [한 줄] |
| 글 구조를 기계가 이해하도록 설정되어 있나요? | 좋음 / 주의 / 위험 | [한 줄] |
| 출처(브랜드명)를 명확히 식별할 수 있나요? | 좋음 / 주의 / 위험 | [한 줄] |
...
```markdown
# [페이지 제목] AI Citation Readiness Analysis

Date: [날짜]  |  URL: [URL]

---

## Citation Readiness Score: [점수]/100
...
## Critical Citation Readiness Gaps

### [CRITICAL] FAQPage Schema Missing
현재: FAQ 형식 콘텐츠 존재하나 구조화 데이터 없음
영향: FAQ 항목을 개별 답변 후보로 식별하기 어려워질 수 있음
해결:
  ```json
  {
...
현재: speakable 속성 없음
영향: speakable을 지원하는 표면에서 요약 구간을 식별하기 어려울 수 있음
해결:
  ```json
  {
    "@context": "https://schema.org",
...
| 우선순위 | 작업 | 난이도 | 예상 readiness 효과 | 담당 |
|---|---|---|---|---|
